...
class Cart
{
    ...
    // Fonction qui permet de récupérer les objets product, renvoit un tableau associatif ['product' => Objet product, 'quantity' => quantité]
    public function getFull()
    {
        $cartComplete = [];

        // on récupère le contenu de notre panier
        if ($this->get()) {
            foreach ($this->get() as $id => $quantity) {
                // On récupère l'objet product en BDD via son identifiant
                $product_object = $this->entityManager->getRepository(Product::class)->findOneById($id);

                // Cas ou un faux identifiant a été injecté dans l'URL, on ne trouve pas de produit en BDD, on supprime cette entrée de la session
                if (!$product_object)
                {
                    $this->delete($id);
                    continue;
        ...
        // Pour chaque entrée du tableau session['cart'] avec clé comme $id et valeur comme $quantity
